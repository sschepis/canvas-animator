export class Vector2 {
    ...
    magnitudeTo(v: Vector2): number { return v.sub(this).length; }
    angleTo(v: Vector2): number { return Math.acos(this.dot(v) / (this.length * v.length)); }
    inverse(): Vector2 { return new Vector2(this.y, this.x); }
    rotate(rads: number): Vector2 { return new Vector2(this.x * Math.cos(rads) - this.y * Math.sin(rads), this.x * Math.sin(rads) + this.y * Math.cos(rads)); }
    get normalize(): Vector2 { return this.divideScalar(this.length); }
    get normal(): Vector2 { return this.clone.normalize; }
    get clone(): Vector2 { return new Vector2(this.x, this.y); }
    get magnitude(): number { return this.x / this.y; }
    get toString(): string { return `(${this.x},${this.y})`; }
    get length(): number { return Math.sqrt(this.x * this.x + this.y * this.y); }
    get vec3(): Vector3 { return new Vector3(this.x, this.y, 0); }
    equals(v: Vector2): boolean { return this.x == v.x && this.y == v.y; }
    scalarEquals(s: number): boolean { return this.x == s && this.y == s; }
...
export class Vector3 {
    ...
    get normalize(): Vector3 { return this.divideScalar(this.length); }
    get normal(): Vector3 { return this.clone.normalize; }
    get clone(): Vector3 { return new Vector3(this.x, this.y, this.z); }
    get magnitude(): number { return this.x / this.y; }
    ...
    cloneTo2d( camera: Vector3, screen: Vector2, rotation: Vector3 = new Vector3(1, 1, 1), scale: number = 100 ): Vector2 { return this.projectTo2d(camera, screen, rotation, scale); }
